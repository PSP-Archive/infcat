...
// RFC1321 "APPENDIX A - Reference Implementation"を参考に修正したものです。
...
*/
static void
Encode( uint8_t* output, const uint32_t* input, size_t len )
{
	size_t i;
	size_t j;
...
*/
static void
Decode( uint32_t* output, const uint8_t *input, size_t len )
{
	size_t i;
	size_t j;
	for (i = 0, j = 0; j < len; i++, j += 4) {
...
*/
static void
MD5Transform( uint32_t state[4], const uint8_t* block )
{
...
*/
void
Cat_MD5Update( MD5_CTX* context, const uint8_t* input, size_t inputLen )
{
	uint32_t i;
	uint32_t index;
	uint32_t partLen;
...
*/
void
Cat_MD5Final( MD5_CTX* context, uint8_t digest[16] )
{
	uint8_t bits[8];
	uint32_t index;
	uint32_t padLen;
	...
	Cat_MD5Update( context, PADDING, padLen );
	/* Append length (before padding) */
	Cat_MD5Update (context, bits, 8);
